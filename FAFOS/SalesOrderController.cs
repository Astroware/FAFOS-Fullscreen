...
namespace FAFOS
{
    public class SalesOrderController
    {
        ...
        public void createSalesOrder(object sender, EventArgs e)
        {
            ...
            {
                if (dt.Rows[i].Cells[0].Value != null && dt.Rows[i].Cells[1].Value != null && (dt.Rows[i].Cells[3].Value != null || dt.Rows[i].Cells[4].Value != null))
                {
                    valid = true;
                }
                else
            ...
            if (valid == false)
            {
                MessageBox.Show("Form is incomplete");
            }
        ...
        public void dgvSalesOrder_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                String a = dgv.Rows[e.RowIndex].Cells[4].Value.ToString();
                double n;
                bool isNumeric = double.TryParse(a, out n);
                if (double.TryParse(a, out n))
                {
            ...
            {
                String a = dgv.Rows[e.RowIndex].Cells[3].Value.ToString();
                double n;
                bool isNumeric = double.TryParse(a, out n);
                if (double.TryParse(a, out n))
                {
        ...
        public void previewEdit(object sender, EventArgs e)
        {
            Sales_Order createSalesOrder = (Sales_Order)((Button)sender).FindForm();
            DataGridView dt = createSalesOrder.getOrderItems();
            bool valid = false;
            for (int i = 0; i < dt.Rows.Count - 1; i++)
            {
                if (dt.Rows[i].Cells[0].Value != null && dt.Rows[i].Cells[1].Value != null && dt.Rows[i].Cells[3].Value != null && dt.Rows[i].Cells[4].Value != null)
