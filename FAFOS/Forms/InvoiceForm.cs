...
namespace FAFOS
{
    public partial class InvoiceForm : FAFOS.Background
    {
        ...
        Users user;
        private int userid;
        private static String franchiseeUserId; //for quotes
        private static String franchiseeId;
        public static int saleQuote;//checks if its a sale or quote item. Its for previewing
        //public static int saleQuote2=1;
        public InvoiceForm(int id)
        {
            InitializeComponent();

            franchiseeUserId = id.ToString();
            franchiseeId = new Users().getFranchiseeId(id.ToString());
            this.fillQuoteIDs(new Quote().getInProgress(franchiseeUserId));
            ...
            this.Find_btn_Quote.Click += new System.EventHandler(my_controller.Find_btn_Quote_Click);
            ...
            //combobox datasource for quotes
         /*   DataTable qt = new Quote().getDone(userid.ToString()).Tables[0];
            //DataTable qt = new Quote().get().Tables[0];
            DataTable quotes = qt.Clone();
            quotes.Rows.Add(new String[] { "", "0" });
            foreach (DataRow r in qt.Rows)
                quotes.ImportRow(r);

            txtQuote.DataSource = quotes;
            txtQuote.DisplayMember = quotes.Columns[0].ToString();
            txtQuote.ValueMember = quotes.Columns[1].ToString();*/
        }
        ...
        {
            return this.txtQuote.SelectedValue.ToString();
        }
        ...
        public void Preview(String address, String clientInfo, String franchiseeInfo)
        {
            if (dataGridView1.RowCount > 0)
            {
                ...
                {
                    MessageBox.Show("Incorrect Information. Could not display the document.");
                }
            }
            else
            {
        ...
        {
            txtQuote.DataSource = ts;
            txtQuote.DisplayMember = ts.Columns[0].ToString();
            txtQuote.ValueMember = ts.Columns[0].ToString();
        }
        ...
        public void fillData(string serviceAddressId, DataTable quoteItems)
        {
            ...
            for (int i = 0; i < quoteItems.Rows.Count; i++)
            {
                ...
                dataGridView1.Rows[index].Cells[0].Value = quoteItems.Rows[i][0];
                dataGridView1.Rows[index].Cells[1].Value = quoteItems.Rows[i][1];
                dataGridView1.Rows[index].Cells[2].Value = new Item().getDescription(dataGridView1.Rows[index].Cells[1].Value.ToString());
                dataGridView1.Rows[index].Cells[1].Value = new Item().getItem(dataGridView1.Rows[index].Cells[1].Value.ToString());
                dataGridView1.Rows[index].Cells[3].Value = quoteItems.Rows[i][3];
                dataGridView1.Rows[index].Cells[4].Value = quoteItems.Rows[i][2];
                dataGridView1.Rows[index].Cells[5].Value = quoteItems.Rows[i][4];

                if (dataGridView1.Rows[index].Cells[3].Value.ToString().Length==0)
                {
                    double a = Convert.ToDouble(dataGridView1.Rows[i].Cells[4].Value);
                    double b = Convert.ToDouble(dataGridView1.Rows[i].Cells[5].Value);
                    dataGridView1.Rows[index].Cells[6].Value = a * b;
                }
                if (dataGridView1.Rows[index].Cells[4].Value.ToString().Length == 0)
                {
                    double a = Convert.ToDouble(dataGridView1.Rows[i].Cells[3].Value);
                    double b = Convert.ToDouble(dataGridView1.Rows[i].Cells[5].Value);
                    dataGridView1.Rows[index].Cells[6].Value = a * b;
                } 
            }
            ...
            {
                total += Convert.ToDouble(dataGridView1.Rows[j].Cells[6].Value);
            }
            tax = total * tax;
            this.SetTotalQuote(total, tax);
        }

        public void SetTotalQuote(double total, double tax)
        {
            double a = total + tax;

            txtSub.Text = "$" + String.Format("{0:0.00}", total);
            txtTax.Text = "$" + String.Format("{0:0.00}", tax);
            txtTotal.Text = "$" + String.Format("{0:0.00}", a);
        ...
        {
            double tempo;
            String[] temr =new String[1];
            String temp= txtTax.Text;
            temr=temp.Split('$',' ');
            tempo = Convert.ToDouble(temr[1]);
            return tempo;
        }
    ...
    //                String[] sAd = new String[6];
    //                sAd = new ServiceAddress().get(quoteForm.getServiceAddressId()).Split(','); ;//address.Split(',');

    //                String[] client = new String[9];
    //                client = new Client().get(new ClientContract().getClient(quoteForm.getServiceAddressId())).Split(',');//clientInfo.Split(',');


    //                String[] franchisee = new String[6];
    //                franchisee = new Franchisee().get(new ClientContract().getFranchisee(quoteForm.getServiceAddressId())).Split(',');//franchiseeInfo.Split(',');

                    //String userInfo = new Users().get(userid.ToString());
    //                String[] user = new String[4];
    //                user = new Users().get(franchiseeUserId).Split(',');//userInfo.Split(',');
  ...
  //                  textAndtable.AddText(50, 200, sAd[2], 10, "T1", Align.LeftAlign);
                    ...
                    table2.rowHeight = 15;
                    textAndtable2.SetParams(table2, cell1, Align.CenterAlign, 3);
                    textAndtable2.AddRow(false, 8, "T3", alignR, false, "Net" + txtTerm.Text, txtQuote.SelectedValue.ToString(),
                        Issued.Value.ToShortDateString(), Issued.Value.AddDays(Convert.ToDouble(txtTerm.Text)).ToShortDateString());
                    content.SetStream(textAndtable2.EndTable(line1, true));
